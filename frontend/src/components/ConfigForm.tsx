  ...
  return (
    <div className="card space-y-3">
      ...
      <div className="rounded border border-slate-800 p-3 text-sm text-slate-400">
        S7 安全模式已锁定：TESTNET / DRY-RUN，Mainnet 不支持，Demo 实际下单路由默认关闭。
        Settings 仅管理 Paper 风控与策略范围。
      </div>
      ...
      <label>杠杆（最大 5）<input className="input" type="number" min={1} max={5} value={cfg.default_leverage ?? 1} onChange={(e) => setCfg({ ...cfg, default_leverage: Number(e.target.value) })} /></label>
      <label>单笔风险比例<input className="input" type="number" min={0.0001} max={0.01} step="0.001" value={cfg.risk_per_trade ?? 0.005} onChange={(e) => setCfg({ ...cfg, risk_per_trade: Number(e.target.value) })} /></label>
      <label>每日最大亏损比例<input className="input" type="number" min={0.005} max={0.03} step="0.001" value={cfg.max_daily_loss ?? 0.02} onChange={(e) => setCfg({ ...cfg, max_daily_loss: Number(e.target.value) })} /></label>
