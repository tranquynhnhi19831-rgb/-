...
def generate_jianghe_signals_research_fast(
    ...
    The normal cached runner still invokes every enabled evaluator on each 1m
    bar and lets unsupported evaluators reject the context internally. That is
    simple but expensive over multi-year minute history. This research path
    skips only calls whose result is deterministically non-candidate from the
    evaluator contracts:

    - pullback/breakout are evaluated only in confirmed bull/bear trends;
    - second-push is not evaluated in UNKNOWN context;
    - range second-push is skipped when the runner config disallows it.

    Context visibility, setup configs, candidate construction and cooldown are
    delegated to the same functions used by ``jianghe_runner`` so signal
